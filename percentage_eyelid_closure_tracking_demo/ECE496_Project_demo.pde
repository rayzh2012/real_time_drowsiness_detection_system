...
OpenCV opencv;
 Rectangle []faces;
Capture video;
Rectangle[] eyes= new Rectangle[20];
...
void setup() {
  ...
  video = new Capture(this, round(640), round(480));
  //video = new Capture(this, 640, 480, "USB2.0 PC CAMERA");
  video.start();
  
  opencv = new OpenCV(this, round(640), round(480));
...
void draw() {
  ...
  pushMatrix();
    ...
    rect(faces[i].x+faces[i].width/2,faces[i].y, faces[i].width/2, faces[i].height/2);
...
void detectBlobs() {
  ...
  //Rectangle r Contours detected in this frame
  // Passing 'true' sorts them by descending area.
  contours = opencv.findContours(true, true);
...
ArrayList<Contour> getBlobsFromContours(ArrayList<Contour> newContours) {
  ...
  for (int j=0; j<newContours.size(); j++) {
    ...
    Contour contour = newContours.get(j);
    Rectangle r = contour.getBoundingBox();
    ...
    for (int i = 0; i < faces.length; i++) {
        ...
        if (//(contour.area() > 0.9 * src.width * src.height) ||
        (r.width < blobSizeThreshold || r.height < blobSizeThreshold) &&
         (r.x>faces[i].x) && (r.y>faces[i].y) && (r.x<(faces[i].x+faces[i].width)) &&(r.y<(faces[i].y+faces[i].height/2))
        //(r.width<40)&&(r.height<40)
        &&(abs(r.width-r.height)<10)&&(abs(1-r.width/r.height)<0.5)&&((1-abs(r.width*r.height/(PI*pow(radius,2))))<0.5)&&(abs(radius-radius2)<4))
