...
{ k=d/100*6.35;}
 ...
 // for(int i=0; i<numOrgans; i++){
    myGraph.myPieChart[0].DrawBarGraph(k);
//  }
...
public class Graph {
  ...
  public Graph(int numSlices){
    ...
    myPieChart[0] = new PieSlice("Attention Level",0,color(201,32,39));
    myPieChart[1] = new PieSlice("Drowsiness Levels",1,color(227,120,24));
    myPieChart[2] = new PieSlice("Relaxiation Levels",2,color(227,190,50));
    myPieChart[3] = new PieSlice("Blinking Frequency",3,color(4,130,115));
    myPieChart[4] = new PieSlice("Face/Eye Movement",4,color(94,93,125));
  }
  ...
  public void DrawCenter(int yearIndex){
    ...
    // declare local variables
    String title = "Drowsiness Detection Display";
    ...
    // title
    textSize(24);
    text("Drowsiness Meters", width/2,height/2-36-18);
    text("ECE496 Final Design",width/2,height/2-36+18);
    ...
    textFont(myFontB);
    textSize(48);
    text(round(k/6.35*100)+"% Drowsy",width/2,height/2+14);
    rectMode(CORNER);
    textFont(myFont);
...
class PieSlice {
  ...
  //********************************************************
  public void DrawBarGraph(float yearIndex){
    // local variables
    float aStart = radians(angleZero+angleStart);
    float aEnd = aStart+radians(angleWidth);
    float bWidth;
    ...
    //  bWidth = (float)(myDataSet[yearIndex][i][orgIndex])/totalYearOrgRemovals*(aEnd-aStart)*scalingFactor; // in radians
     // bStart = aEnd-bWidth; // in radians
      for(int j=1; j<myGraph.ringWidth; j++){
        ...
        noFill();
        arc(myGraph.x,myGraph.y,myGraph.outerDiameter-(i*myGraph.ringWidth)-j-1,myGraph.outerDiameter-(i*myGraph.ringWidth)-j-1,0,yearIndex);
      }
    }
  }
}
